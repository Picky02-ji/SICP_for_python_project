...
def extract_words(text):
    ...
    res = []
    while i < len(text):
        while i < len(text) and (text[i] < 'a' or text[i] > 'z') and (text[i] <'A' or text[i] > 'Z'):
            i += 1
        if(i >= len(text)): break
        str = text[i]
        i += 1
        while i < len(text) and (text[i] >= 'a' and text[i] <= 'z' or text[i] >= 'A' and text[i] <='Z'):
            str += text[i]
            i += 1
        res.append(str)
    return res
...
def analyze_tweet_sentiment(tweet):
    ...
    sentiments = 0
    for word in words:
        senti = get_word_sentiment(word)
        if not has_sentiment(senti):
            lenth -= 1
            continue
        sentiments += sentiment_value(senti)
    if lenth == 0:
        return make_sentiment(None)
    return make_sentiment(sentiments / lenth)
...
def find_centroid(polygon):
    ...
    l = len(polygon)
    for i in range(l-1):
        vx, vy = latitude(polygon[i]), longitude(polygon[i])
        vx2, vy2 = latitude(polygon[i+1]), longitude(polygon[i+1])
        S += vx*vy2-vy*vx2
        x += (vx+vx2)*(vx*vy2-vx2*vy)
        y += (vy+vy2)*(vx*vy2-vx2*vy)
    S = S / 2
    if S == 0:
...
def find_state_center(polygons):
    ...
    for poly in polygons:
        A += find_centroid(poly)[2]
    for poly in polygons:
        center = find_centroid(poly)
        x += center[0] * (center[2] / A)
        y += center[1] * (center[2] / A)
...
def group_tweets_by_state(tweets):
    ...
    "*** YOUR CODE HERE ***"
    states = {}
    for s, p in us_states.items():
        states[s] = find_state_center(p)
    for tweet in tweets:
        s = get_nearest(states, tweet)
        if not s in tweets_by_state.keys():
            tweets_by_state[s] = []
        tweets_by_state[s] += [tweet]
    return tweets_by_state

def get_nearest(states, tweet):
    res = 'None'
    val = 0
    flag = False
    for state, pos in states.items():
        tmp = geo_distance(pos, tweet_location(tweet))
        if not flag:
            flag = True
            val = tmp
        if tmp < val:
...
def average_sentiments(tweets_by_state):
    ...
    for state, tweets in tweets_by_state.items():
        sentiments = get_average_tweets_sentiments(tweets)
        if(has_sentiment(sentiments)):
            averaged_state_sentiments[state] = sentiment_value(sentiments)
    return averaged_state_sentiments

def get_average_tweets_sentiments(tweets):
    size = 0
    sentiments = 0
    for tweet in tweets:
        senti = analyze_tweet_sentiment(tweet)
        if(has_sentiment(senti)):
            size += 1
            sentiments += sentiment_value(senti)
    if size == 0:
        return make_sentiment(None)
    return make_sentiment(sentiments / size)
