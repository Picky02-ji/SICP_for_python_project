...
# my own func for get max in a nums
def getMax(score):
    s = score
    res = 0
    while s:
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    if num_rolls == 0:
        return getMax(opponent_score) + 1
    else:
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score < 100 and opponent_score < 100:
        if(who == 0):
            nums = strategy0(score, opponent_score)
            score += take_turn(nums, opponent_score, select_dice(score, opponent_score))
        else:
            nums = strategy1(opponent_score, score)
            opponent_score += take_turn(nums, score, select_dice(score, opponent_score))
        if(score == opponent_score * 2 or score * 2 == opponent_score):
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    """
    "*** YOUR CODE HERE ***"
    stretegy = make_averaged(roll_dice)
    maxx = 0
    for i in range(1, 11):
        score = stretegy(i, dice)
        print('%d dice scores %.1f on average' % (i, score))
...
def bacon_strategy(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    maxx = getMax(opponent_score)
    if(maxx + 1 >= BACON_MARGIN):
        return 0
...
def swap_strategy(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    bacon_score = getMax(opponent_score) + 1
    if(2 * (bacon_score + score) == opponent_score):
        return 0
    if(bacon_score + score == opponent_score * 2):
        return BASELINE_NUM_ROLLS
    return bacon_strategy(score, opponent_score)

def final_strategy(score, opponent_score):
    """First consider let opponent get more Hog wild situation. If I'm in 
    ...
    """
    "*** YOUR CODE HERE ***"
    bacon = getMax(opponent_score)
    if(score + bacon + 1 + opponent_score % 7 == 0): return 0
    if(score >= opponent_score):
        if(bacon > 5 and score + 1 + bacon != 2 * opponent_score):
            return 0
        if((score + opponent_score) % 7 == 0):
            return 4
        return 6
    else:
        if(bacon > 8 or 2 * (score + 1 + bacon) == opponent_score):
            return 0
        if((score + opponent_score) % 7 == 0):
            return 4
        return 6
...
def run_experiments():
    ...
    print('win rate to always_roll(5):', average_win_rate(final_strategy, always_roll(5)))

    if True: # Change to True to test always_roll(8)
        print('always_roll(8) win rate:', average_win_rate(final_strategy, always_roll(8)))

    if True: # Change to True to test bacon_strategy
        print('bacon_strategy win rate:', average_win_rate(final_strategy, bacon_strategy))

    if True: # Change to True to test swap_strategy
        print('swap_strategy win rate:', average_win_rate(final_strategy, swap_strategy))

    if True: # Change to True to test final_strategy
        print('final_strategy win rate:', average_win_rate(final_strategy, final_strategy))
